        ...
        <>
          {tasks.length > 0 ? (
            <View style={styles.tasksList}>
                     {tasks.map(task => {
  if (task.progress == null || task.progress < 100) { // Only display tasks with progress 100% or more
    return (
      <ScrollView>
<View key={task.id} style={styles.taskCard}>
        ...
        <TouchableOpacity
         onPress={() => {
          Alert.alert(
            ...
            [
              { text: "High", onPress: () => handlePriorityChange(task.id, "High") },
              { text: "Medium", onPress: () => handlePriorityChange(task.id, "Medium") },
              { text: "Low", onPress: () => handlePriorityChange(task.id, "Low") },
            ],
            { cancelable: true }
          );
        }}
        ...
        >
          <Text style={styles.priorityButtonText}>Set Priority</Text>
        </TouchableOpacity>
      </View>
      </ScrollView>
